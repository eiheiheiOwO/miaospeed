...
// Author: https://github.com/jpillora/ipfilter [MIT License]
// Modified by: https://github.com/AirportR [MIT License]
import (
...
// Options
//
//	for IPFilter. Allow supercedes Block for IP checks
//
...
func (f *IPFilter) ToggleIP(str string, allowed bool) bool {
	//check if has subnet
	if ip, _net, err := net.ParseCIDR(str); err == nil {
		// containing only one ip? (no bits masked)
		if n, total := _net.Mask.Size(); n == total {
			f.mut.Lock()
